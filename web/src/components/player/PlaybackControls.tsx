...
export default function PlaybackControls({
  ...
  const ctlBtn =
    "text-ink w-9 h-9 max-nav:w-11 max-nav:h-11 rounded-full text-[15px] flex items-center justify-center transition-[color,background,transform] duration-200 cursor-none hover:text-aqua hover:bg-aqua/[.08] active:scale-90 focus-visible:outline-none focus-visible:shadow-glow-aqua";
  const skipBtn = ctlBtn + " text-[11px] font-mono";
  const playBtn =
    "w-12 h-12 max-nav:w-[52px] max-nav:h-[52px] rounded-full bg-aqua text-[#04100E] text-[18px] flex items-center justify-center transition-[transform,background] duration-200 cursor-none shadow-[0_6px_18px_rgba(56,232,206,.3)] hover:scale-[1.07] hover:bg-[#6FF3DE] active:scale-[.94] focus-visible:outline-none focus-visible:shadow-glow-aqua";
  ...
  return (
    <div className="flex flex-col gap-2.5 items-center w-[min(560px,44vw)] max-nav:w-full">
      ...
      <div className="flex items-center gap-3 w-full">
        <span className="mono !text-[9.5px] flex-none w-8 text-right">{fmt(time)}</span>
        <div
        ...
        <span className="mono !text-[9.5px] flex-none w-8">{fmt(dur)}</span>
      </div>
    </div>
  );
}
