...
/* WebGL (ogl) дугуй галерей — landing хуудасны хуучин scroll-jack CSS slide-track
   (#gal/#track/#galbar/#galno, landing-engine.js-ээр удирдагдаж байсан) -ийг орлов.
   Landing-engine.js доторх тэдгээр DOM хаягуудыг ашигладаг код (§ galOuter/track/
   galbar/galno) одоо элемент олохгүй тул null-check-ээр хамгаалагдсан — алдаа өгөхгүй,
   зүгээр л скролл-жак эффект идэвхгүй болно (доор landing-engine.js-д тэмдэглэсэн). */
...
class App {
  ...
  constructor(
    container: HTMLElement,
    {
      ...
      textColor = "#ffffff",
      borderRadius = 0,
      font = "600 30px 'Golos Text', system-ui, sans-serif",
      scrollSpeed = 2,
      scrollEase = 0.05,
  ...
  createMedias(items: GalleryItem[] | undefined, bend = 1, textColor: string, borderRadius: number, font: string) {
    // Давхарлаж (x2) endless-loop эргэдэг болгоно — Media.update()-ийн isBefore/isAfter
    // wrap логик зөв ажиллахын тулд жагсаалт хоёр дахин орох шаардлагатай.
  ...
  onWheel(e: any) {
    // Container дотроос гарсан wheel эвент л галерейг эргүүлнэ — хуудасны бусад
    // хэсэгт хулгана байрлаж байхад landing page scroll-той зөрчилдөхгүй.
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    /* Гарчгийг canvas дээр нэг л удаа зурдаг тул вэб-фонт ачаалагдахаас өмнө эхэлбэл
       fallback (serif) фонтоор "хөлдөнө" — document.fonts.ready-г хүлээж байж эхлүүлнэ. */
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Дугуй зургийн галерей. Зүүн/Баруун сум товч ашиглан удирдана уу."
      ref={containerRef}
    />
  );
}
