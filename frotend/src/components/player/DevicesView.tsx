    ...
    <>
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-3.5 mb-3">
        {devices.map((d) => (
          <div
            key={d.key}
            className={
              "flex flex-col items-start gap-2 rounded-2xl p-5 transition-[background,box-shadow,transform] duration-250 hover:-translate-y-0.5 hover:shadow-sm " +
              (d.ok ? "bg-aqua/[.08] hover:bg-aqua/[.11]" : "bg-white/[.035] hover:bg-white/[.055]")
            }
          >
      ...
      <div className="flex flex-col gap-3 mb-5">
        {['bass', 'mid', 'high'].map((band) => (
          <div className="flex items-center gap-3 bg-white/[.035] rounded-xl py-2.5 px-3.5" key={band}>
            <button
