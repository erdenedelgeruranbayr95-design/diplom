...
import type { ViewName, Prefs } from "@/components/providers/PlayerProvider";
...
export default function TopBar({
  ...
  return (
    <>
      <header className="relative z-[6] flex items-center gap-6 max-nav:gap-3 h-[64px] px-5 max-nav:px-3 bg-transparent">
        <span className="flex-none font-display font-extrabold text-[18px] max-nav:text-[15px] tracking-[-.04em] whitespace-nowrap [&>sup]:font-body [&>sup]:text-[9px] [&>sup]:font-medium [&>sup]:ml-0.5">
          МЭДРЭХ<sup>®</sup>
          {isAdmin && (
        ...
        <div className="flex-1 flex items-center justify-center gap-3.5 min-w-0">
          <button
            className={
              "w-[42px] h-[42px] max-viz:w-[38px] max-viz:h-[38px] max-nav:hidden flex-none rounded-full flex items-center justify-center transition-[color,background,box-shadow] duration-250 cursor-none focus-visible:outline-none focus-visible:shadow-glow-aqua disabled:opacity-40 disabled:cursor-not-allowed disabled:pointer-events-none " +
          ...
          <div className="flex-1 min-w-0 max-w-[460px] max-viz:max-w-none mx-auto flex items-center gap-3 max-nav:gap-2 h-11 bg-white/[.05] border border-white/[.06] rounded-full px-4 max-nav:px-3 text-dim transition-[border-color,background,box-shadow] duration-300 focus-within:bg-white/[.08] focus-within:border-aqua/60 focus-within:shadow-glow-aqua">
            ...
            <input
              ...
              value={query}
              /* Хайлтын үр дүн каталогтой хамт Судлах дээр гардаг тул тэр рүү шилжүүлнэ */
              ...
              aria-label="Дуу хайх"
              className="flex-1 min-w-0 w-full bg-transparent border-none text-ink font-body text-[14px] cursor-none outline-none placeholder:text-faint [&::-webkit-search-cancel-button]:hidden [&::-webkit-search-decoration]:hidden appearance-none"
            />
          </div>
