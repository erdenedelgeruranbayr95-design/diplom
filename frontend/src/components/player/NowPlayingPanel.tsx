    ...
    >
      <div className="max-w-[1180px] mx-auto p-5 max-nav:p-4">
        <div className="flex items-center justify-between gap-4 mb-5">
          <div>
            <span className="mono block mb-1">NOW PLAYING</span>
            <div className="flex items-center gap-3 flex-wrap">
              <h3 className="font-display font-semibold text-[24px] max-nav:text-[20px] tracking-[-.04em] leading-tight text-ink">
                {track.title}
              </h3>
              <span className={"inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-medium " + (vibro ? "border-aqua/30 bg-aqua/[.09] text-aqua" : "border-white/[.1] bg-white/[.04] text-dim")}>
        ...
        <div className="grid grid-cols-[1.1fr_.9fr] max-nav:grid-cols-1 gap-6 items-start">
          <div className="rounded-[28px] border border-white/[.08] bg-white/[.03] p-4">
            <div className="flex items-center gap-3 mb-4">
              <img src={track.cover || ""} alt="" className="w-14 h-14 rounded-2xl object-cover flex-none shadow-[0_8px_24px_rgba(0,0,0,.35)]" />
              <div className="min-w-0">
                <b className="block text-[15px] font-semibold text-ink whitespace-nowrap overflow-hidden text-ellipsis">{track.title}</b>
                <i className="block text-[12.5px] text-dim not-italic whitespace-nowrap overflow-hidden text-ellipsis">
                  {track.artist} · {track.genre}
                </i>
              </div>
            </div>

            <div className="flex items-end gap-2 h-[104px] border border-line rounded-[18px] p-3.5 bg-black/[.22]" aria-label="Амьд давтамжийн спектр">
            ...
            <div className="mt-4 flex flex-col gap-2.5">
              ...
              <div className="flex items-center h-[26px] border border-line rounded-[9px] px-2.5 bg-[rgba(20,28,27,.4)]" aria-hidden="true">
                {f.pattern.map((ms, i) =>
                  i % 2 === 0 ? (
                    <i key={i} className="block h-3 rounded-[3px] bg-aqua shadow-[0_0_8px_rgba(56,232,206,.4)]" style={{ flex: ms / tot + " 0 0" }} />
                  ) : (
          ...
          <div className="flex flex-col gap-4">
            <div className="rounded-[28px] border border-white/[.08] bg-white/[.03] p-4">
              <div className="flex gap-2.5 flex-wrap [&>button]:flex-1 [&>button]:justify-center">
                <button
                  className={
                    "inline-flex items-center justify-center gap-2 text-[12.5px] rounded-full border py-[9px] px-[15px] whitespace-nowrap transition-[border-color,color,background] duration-300 " +
            ...
            <div className="rounded-[28px] border border-white/[.08] bg-white/[.03] p-4">
              <div className="flex items-center justify-between gap-3 mb-3">
                <div>
                  <h4 className="font-display font-semibold text-[16px] tracking-[-.03em] text-ink">Device sync</h4>
                  <p className="text-[12px] text-dim mt-0.5">Холболтын байдал ба test vibration</p>
                </div>
                <span className="mono !text-[9px]">{canVibrate ? "VIB OK" : "NO VIB"}</span>
              </div>
