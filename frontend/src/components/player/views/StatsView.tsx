...
/* Миний статистик. Тоглуулах үйлдэл нь контекстээс — өмнө нь `byId` хайгч функц болон
   `onPlay` callback хоёрыг Player-ээс prop-оор дамжуулдаг байв. */
...
export default function StatsView({
  ...
  const topTracks = Object.entries(s.byTrack).sort((a, b) => b[1] - a[1]).slice(0, 5)
    .map(([id, sec]) => ({ t: trackIndex.get(id), sec })).filter((x): x is { t: PlayerTrack; sec: number } => !!x.t)
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-7 gap-2.5 h-[180px] items-end border border-white/[.08] rounded-2xl p-[18px_18px_12px] bg-white/[.02]">
        {days.map((d, i) => (
          <div className="flex flex-col items-center gap-[7px] h-full justify-end" key={i}>
            <span className="mono !text-micro">{d.sec ? fmtDur(d.sec) : ""}</span>
            <i
              className={
        ...
        <>
          ...
          <div className="flex flex-col">
            {topTracks.map(({ t, sec }, i) => (
              <button
                key={t.id}
                className="grid grid-cols-[34px_44px_1fr_auto_30px] gap-3 items-center py-2.5 px-3.5 mb-0.5 rounded-chip text-ink text-left transition-colors duration-250 hover:bg-white/5"
                onClick={() => play(t)}
              >
                <span className="mono !text-meta">{'0' + (i + 1)}</span>
                <img className="w-11 h-11 rounded-lg object-cover shadow-[0_4px_12px_rgba(0,0,0,.3)]" src={t.cover} alt="" loading="lazy" decoding="async" />
                ...
                <span className="mono !text-micro max-nav:hidden">{fmtDur(sec)}</span>
      ...
      {s.total === 0 && <p className="text-faint text-body text-center py-6 px-4">Дуу сонсож эхлэхэд статистик энд цуглана</p>}
    </>
  )
}
